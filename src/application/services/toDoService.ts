...
class ToDoService {
  ...
  findAll(
    ...
    Logger.debug(
      `ToDoService - findAll - call toDoRepository.findAll with page: ${page} and limit: ${limit}`
    );
  ...
  async updateDescription(id: string, body: string): Promise<void> {
    ...
    if (task.completed) {
      throw new UnprocessableEntityError(
        'Cannot update description of a completed task'
      );
    }

    Logger.debug(
      'ToDoService - update - call toDoRepository.updateDescription'
    );
  ...
  async updateStatus(id: string, completedStatus: boolean): Promise<void> {
    ...
    Logger.debug(
      'ToDoService - updateStatus - call toDoRepository.updateStatus'
    );
